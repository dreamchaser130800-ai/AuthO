...
<head>
    ...
    <title>@yield('title', 'Organisasi - AmikomEventHub')</title>
    ...
    <aside class="w-64 bg-indigo-900 text-indigo-100 flex flex-col p-6 space-y-8 sticky top-0 h-screen">
        <div class="flex items-center gap-3">
            <div
                ...
                {{ substr(Auth::guard('organization')->user()->name, 0, 1) }}
            </div>
            <span class="text-xl font-bold text-white tracking-tight">{{ Auth::guard('organization')->user()->name }}</span>
        </div>
        ...
        <nav class="flex-1 space-y-2">
            ...
            <a href="{{ route('org.dashboard') }}"
                class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('org.dashboard') ? 'bg-indigo-800 text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
            ...
            <a href="{{ route('org.events.index') }}"
                class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('org.events.*') ? 'bg-indigo-800 text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
